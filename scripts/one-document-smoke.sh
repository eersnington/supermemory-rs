#!/usr/bin/env bash
# Runs one live extraction job and prints only its structured timing/error logs.
set -Eeuo pipefail

: "${GOOGLE_API_KEY:?set GOOGLE_API_KEY before running this smoke test}"
ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)
DATA=$(mktemp -d /tmp/supermemory-one-document.XXXXXX)
LOG=$(mktemp /tmp/supermemory-one-document.XXXXXX.log)
PORT=${SUPERMEMORY_SMOKE_PORT:-6769}
PID=""
...
created=$(curl -fsS -X POST "http://127.0.0.1:$PORT/v3/documents" \
  ...
  -d '{"content":"Caroline attended an LGBTQ support group on 7 May 2023.","customId":"smoke-lgbtq-2023-05-07","metadata":{"date":"2023-05-07"}}')
...
result=$(curl -fsS -X POST "http://127.0.0.1:$PORT/v4/search" \
  -H 'content-type: application/json' \
  -d '{"q":"When did Caroline attend the LGBTQ support group?","limit":10,"searchMode":"memories"}')
printf '%s' "$result" | python3 -c 'import json,sys; data=json.load(sys.stdin); print(json.dumps(data)); assert any("LGBTQ" in item["memory"] for item in data["results"])'
