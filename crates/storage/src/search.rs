...
impl Storage {
    ...
    pub fn search_memory_candidates(
        ...
        validate_vector(query, query.len())?;
        let mut statement = self.connection.prepare(
            "WITH ranked AS MATERIALIZED (SELECT memories.id, vector_dot(memory_embeddings.vector, ?1, ?2) AS score FROM memory_embeddings JOIN memories ON memories.id=memory_embeddings.memory_id WHERE memories.org_id=?3 AND memories.container_tag=?4 AND memory_embeddings.model_id=?5 AND memory_embeddings.dimensions=?2 AND (memory_embeddings.active=1 OR ?6=1) AND (memories.is_latest=1 OR ?7=1) AND (memories.is_forgotten=0 OR ?8=1) AND (memories.forget_after IS NULL OR datetime(memories.forget_after)>CURRENT_TIMESTAMP OR ?9=1)) SELECT id, score FROM ranked WHERE score>=?10 ORDER BY score DESC, id LIMIT ?11",
        ).map_err(StorageError::Read)?;
        statement
            .query_map(
                params![
    ...
    pub fn search_chunk_candidates(
        ...
        let metadata_filter = options.filters.as_ref().map(json).transpose()?;
        let mut statement = self.connection.prepare(
            "WITH ranked AS MATERIALIZED (SELECT document_chunks.id, document_chunks.stable_id, documents.id AS document_id, document_chunks.ordinal, vector_dot(chunk_embeddings.vector, ?1, ?2) AS score FROM chunk_embeddings JOIN document_chunks ON document_chunks.id=chunk_embeddings.chunk_id JOIN documents ON documents.id=document_chunks.document_id WHERE chunk_embeddings.model_id=?3 AND chunk_embeddings.dimensions=?2 AND documents.org_id=?4 AND documents.status='done' AND (?5='[]' OR EXISTS(SELECT 1 FROM json_each(documents.container_tags) stored JOIN json_each(?5) requested ON stored.value=requested.value)) AND (?6 IS NULL OR documents.id=?6 OR documents.custom_id=?6) AND (?7 IS NULL OR documents.filepath=?7 OR (substr(?7, -1)='/' AND substr(documents.filepath, 1, length(?7)-1)=substr(?7, 1, length(?7)-1))) AND (?8 IS NULL OR matches_metadata_filter(documents.metadata, ?8))) SELECT id, stable_id, document_id, ordinal, score FROM ranked WHERE score>=?9 ORDER BY score DESC, document_id, ordinal, stable_id LIMIT ?10",
        ).map_err(StorageError::Read)?;
        statement
            .query_map(
                params![
